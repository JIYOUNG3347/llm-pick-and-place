...
```mermaid
flowchart TD
    I["instruction"] --> P["Perception<br/>perceive()"]
    P -->|WorldState| L["Planner<br/>plan()"]
    L -->|"Plan = SkillCall sequence"| E["SkillExecutor<br/>act → env.step → perceive"]
    E -->|ActionResult| C{"all skills<br/>succeeded?"}
    C -->|Yes| S["RunResult<br/>success=True"]
    C -->|"fail · budget left"| P
```
